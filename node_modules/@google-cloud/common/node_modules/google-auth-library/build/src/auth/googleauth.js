...
class GoogleAuth {
    ...
    async getClient(options) {
        ...
        if (!this.cachedCredential) {
            if (this.jsonContent) {
                this.cachedCredential = await this.fromJSON(this.jsonContent, this.clientOptions);
            }
            else if (this.keyFilename) {
                const filePath = path.resolve(this.keyFilename);
                const stream = fs.createReadStream(filePath);
                this.cachedCredential = await this.fromStreamAsync(stream, this.clientOptions);
            }
            else {
